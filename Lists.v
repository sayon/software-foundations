...
Fixpoint count (v:nat) (s:bag) : nat := 
  match s with
      | x::xs => if (eqn x v) then 1 + count v xs else count v xs
      | nil => 0
  end.
...
Definition member (v:nat) (s:bag) : bool := negb (eqn ( count v s ) 0).
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  ...
  match s with 
      | x :: xs => if (eqn v x) then xs else x :: (remove_one v xs)
      | nil => nil
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
      | x :: xs => if (eqn x v) then remove_all v xs else x :: (remove_all v xs)
      | nil => nil
  end.
...
Theorem nat_eq_simmetry: forall n : nat,
                           eqn n n = true.
Proof.
induction n.
reflexivity.
...
Theorem count_add: forall (b: bag) (i :nat),
                     1 + (count i b) = count i (add i b).
Proof.
intros.
induction i.
reflexivity.
simpl.
rewrite nat_eq_simmetry.
reflexivity.
Qed.
...
Theorem same_len_eq: forall (a b: natlist), a =b  -> length a = length b.
Proof.
intros.
induction a.
rewrite H.
...
Theorem rev_cons: forall (n:nat) ( l:natlist) ,
...
Theorem rev_inj:  forall (l1 l2 : natlist), 
           rev l1 = rev l2 -> l1 = l2.
intros.
assert(res: rev( rev l1) = rev( rev l2)).
rewrite H.
reflexivity.
assert (l1r: l1 = rev( rev l1)). rewrite rev_involutive. reflexivity.

assert (l2r: l2 = rev( rev l2)). rewrite rev_involutive. reflexivity.
...
Fixpoint index' (n:nat) (l:natlist) : natoption :=
  match l with
  | nil => None 
  | a :: l' => if eqn n O then Some a else index' (pred n) l'
  end.
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  match l1, l2 with
      | x::xs, y::ys => if (eqn x y) then beq_natlist xs ys else false
      | nil, nil => true 
      | _, _ => false
  end.
