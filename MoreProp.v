...
Inductive total_relation: nat -> nat -> Prop := 
  | any_pair : forall n m: nat, total_relation n m.
...
(** Define an inductive binary relation [empty_relation] (on numbers)
    that never holds. *)
Definition empty (n m: nat) := lt 0 0.
...
Lemma ble_nat_0: forall n,
  ble_nat n 0 = true -> n <= 0.
Proof.  induction n. reflexivity. intro H. inversion H. Qed.

Lemma le_0__eq_0: forall n,
   n <= 0 -> n = 0.
Proof. induction n. reflexivity. intro H. inversion H. Qed.
...
Lemma oddb_neg: forall n, oddb n = true -> evenb n = false.
  intros n H.
  unfold oddb in H.
...
Lemma evenb_neg: forall n, evenb n = true -> oddb n = false.
  intros n H.
  unfold oddb.
  destruct (evenb n).
  reflexivity.
...
Fixpoint true_upto_n__true_everywhere (n:nat) (f: nat->Prop) : Prop :=
  match n with
  | 0 => (forall m: nat, f m)
  | S m => (f n) -> true_upto_n__true_everywhere m f
  end.
